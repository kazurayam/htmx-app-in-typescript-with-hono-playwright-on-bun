// tests/section8.test.ts
import { describe, test, expect, beforeAll, afterAll, beforeEach, afterEach } from 'bun:test';
import * as PW from '@playwright/test';
import { BrowserDriverChromium } from './BrowserDriverChromium';
import { getLogger } from '@logtape/logtape';

const logger = getLogger(["my-app", "section8.test"]);
const url = 'http://localhost:3001/section8';
...
describe(`test ${url}`, async () => {
    ...
    let page: PW.Page;
    beforeAll(async () => {
        driver = await BrowserDriverChromium.create('section8');
    });
    beforeEach(async () => {
     ...
     */
    test("hx-trigger=every 1s", async () => {
    ...
    test("hx-trigger=load, click delay: 0.5s", async () => {
